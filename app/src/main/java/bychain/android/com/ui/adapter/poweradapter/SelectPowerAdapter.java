package bychain.android.com.ui.adapter.poweradapter;
...
public abstract class SelectPowerAdapter<Select extends ISelect> extends PowerAdapter<Select> implements
    ...
    protected boolean isSelectMode;//是否可以使用选择功能
    @Nullable
                ...
                holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        ...
                        return !(currentPos < 0 || currentPos >= list.size())
                            && performLongClick(holder,v, holder.getAdapterPosition(), getItem(currentPos));
                    }
                });
            }
        }
    ...
    @Override
    public void performClick(@NonNull final PowerHolder<Select> powerHolder,@NonNull View itemView, final int position,
                             @NonNull Select select) {
        if (isSelectMode) {
            handleClick(powerHolder, position, select);
        } else {
            if (clickListener != null) {
                clickListener.onItemClick(powerHolder,itemView, position, select);
            }
        }
    }

    @Override
    public boolean performLongClick(@NonNull PowerHolder<Select> holder,@NonNull View itemView, int position, @NonNull Select select) {
        if (longTouchEnable) {
            updateSelectMode(true);
            handleClick(holder, position, select);
            return true;
        } else {
            return super.performLongClick(holder,itemView, position, select);
        }
    }

    @Override
    public void setOnItemSelectListener(AdapterSelect.OnItemSelectedListener<Select> listener) {
