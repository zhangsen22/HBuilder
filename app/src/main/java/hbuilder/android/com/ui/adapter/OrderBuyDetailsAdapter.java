...
public class OrderBuyDetailsAdapter extends PowerAdapter<MySellOrBuyinfoItem> {
    ...
    private class OrderSellDetailsHolder extends PowerHolder<MySellOrBuyinfoItem> {
        ...
        public void onBind(@NonNull final MySellOrBuyinfoItem myBuyinfoItem, int position) {
            ...
            tvOrderSellPrice.setText(new DecimalFormat("0.00").format(myBuyinfoItem.getPrice()));
            tvOrderSellAmount.setText(new DecimalFormat("0.00").format(myBuyinfoItem.getPrice()*myBuyinfoItem.getNum()));
            tvOrderSellPaycode.setText(new DecimalFormat("0.00").format(myBuyinfoItem.getNum()));
            ...
            if(childType == 1){
                if(status == 1){
                    ...
                    tvOrderSellSs.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            BusinessBuyDetailsActivity.startThis(mContext, new BuyBusinessResponse(myBuyinfoItem.getTradeid(), myBuyinfoItem.getPayCode(),myBuyinfoItem.getCreateTime(), GsonUtil.getInstance().objTojson(myBuyinfoItem.getPayee())), myBuyinfoItem.getPrice(), myBuyinfoItem.getNum(), myBuyinfoItem.getPayType());
                        }
                    });
                }else if(status == 2){
