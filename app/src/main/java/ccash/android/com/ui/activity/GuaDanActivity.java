...
public class GuaDanActivity extends BaseActivity {
    ...
    protected void initData() {
        LargeAmountFragment largeAmountFragment = (LargeAmountFragment) getSupportFragmentManager()
                .findFragmentById(R.id.contentFrame);
        if (largeAmountFragment == null) {
            largeAmountFragment = LargeAmountFragment.newInstance("");
            ActivityUtils.addFragmentToActivity(getSupportFragmentManager(),
                    largeAmountFragment, R.id.contentFrame);
        }
        //初始化presenter
        new LargeAmountPresenter(largeAmountFragment, new BuyModle());
    }
}
